...
- Or roll the image first and let one tick run before changing `fleet.json`, with the slot `idle` or `blocked`. The digest still matches, so that tick records the previous template and then refuses with `job_template_changed`. An `active` slot does not write on that tick (a mixed revision would archive two different receipts); the drain to `idle` records the template on the later pass of the same tick. Then record the new digest; the next tick re-keys and launches.
