"""Opaque refresh-safe credential handoff for the Cursor live worker.

The lease bytes are the dedicated Cursor user API key. The native CLI exchanges
it into its own isolated file credential store under the private home and never
rewrites this file, so finish() normally commits identical bytes.
Packaged as /opt/runcrew/app/credential_state.py in the Cursor live image only.
"""
