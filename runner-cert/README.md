...
| `copilot`, `grok` | none | Detection only. The CLI was not installed where this was written, so no flag was verified. The adapter reports `cli_not_found` or `cli_flags_unverified` and never runs the CLI, not even `--version`. |
