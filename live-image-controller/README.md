...
Before submitting either build, stage the image tree with `python live-image-controller/stage_image.py . <Dockerfile|Dockerfile.broker> <out>` and run the Dockerfile's unittest RUN lines with `python -I -B` inside `<out>/opt/runcrew/app` (plus `serve.py --check`, and `--check-broker` for the broker image). The in-image tests see only what the COPY lines copy.
