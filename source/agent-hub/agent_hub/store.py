...
class SQLiteStore:
    ...
    def list_held(self, agent, limit=5):
        """Running rooms whose lease belongs to this agent, including the token."""
        def work(connection):
            return connection.execute(
                "SELECT data FROM rooms WHERE json_extract(data, '$.status')='running' "
                "AND json_extract(data, '$.lease.agent')=? "
                "ORDER BY json_extract(data, '$.updated_at') ASC LIMIT ?",
                (agent, limit)).fetchall()
        return [json.loads(row[0]) for row in self._run(work)]
...
class FirestoreStore:
    ...
    def list(self, agent=None, limit=50):
        ...
        if agent:
            # Match SQLite: only queued work. Deploy needs a composite index on
            # (status, next_agent, created_at) before this query runs in prod.
    ...
    def list_held(self, agent, limit=5):
        ...
        Filters status in memory so the query is a single-field equality on
        lease.agent and does not need a composite index.
        """
        from google.cloud.firestore_v1.base_query import FieldFilter
        query = self.collection.where(filter=FieldFilter('lease.agent', '==', agent)).limit(limit)
        rooms = []
        for document in query.stream():
            room = document.to_dict()
