...
class Hub:
    ...
    def claim(self, actor, room_id=None):
        ...
        reclaimed = self._reclaim(actor, room_id)
    ...
    def _reclaim(self, actor, room_id):
        """Return an in-flight lease when the claim response never arrived.

        Queue scans only list queued rooms, so a lost claim used to leave a
        running room that this agent could not see. Retrying claim returns the
        same token until the lease or deadline actually elapses.
        """
        if room_id is not None:
            rooms = [self.store.get(room_id)]
        else:
            rooms = self.store.list_held(actor)
        for room in rooms:
            if room_id is None and str(room.get('workspace', '')).lower().startswith('release_test_'):
                continue
            task = self._held_task(room, actor)
            if task is not None:
                return task
        return None

    def _held_task(self, room, actor):
        lease = room.get('lease') or {}
        token = lease.get('token')
        expires_at = lease.get('expires_at')
        deadline = lease.get('deadline')
        if (room.get('status') != 'running' or lease.get('agent') != actor
                or not isinstance(token, str) or not token.isascii()):
            return None
        if type(expires_at) not in (int, float) or type(deadline) not in (int, float):
            return None
        now = self.clock()
        if now >= expires_at or now >= deadline:
            return None
        learning_context = copy.deepcopy(lease.get('learning_context') or {})
        return dict(room_id=room['id'], lease_token=token, prompt=room['prompt'],
                    messages=copy.deepcopy(room['messages']), workspace=room['workspace'],
                    timeout_seconds=room['timeout_seconds'], step=room['step'],
                    deadline=deadline, server_time=now,
    ...
    def complete(self, actor, room_id, data):
        ...
        def change(room):
            ...
            now = self.clock()
            # A finish that lands as the lease or deadline elapses is still this
            # worker's result. Expiring first would stall the room and answer 409.
            # Heartbeat, claim, get, and retry still expire before they decide.
            if not self._owns(room, actor, token):
                expire(room, now)
                return None
